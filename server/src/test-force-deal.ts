...
import { CardType, PropertyColor, type GameState, type Card } from "./models/types.ts";
import { createDeck } from "./engine/deck.ts";

function createTestCard(type: CardType, value: number, colors?: PropertyColor[]): Card {
  return {
    id: `test-${Math.random().toString(36).substr(2, 9)}`,
    type,
    value,
    name: type === CardType.Property ? `Test ${colors?.[0]} Property` : `Test ${type}`,
    colors,
  };
}
...
function testForceDeal2Player() {
  ...
  const aliceRedCard = createTestCard(CardType.Property, 3, [PropertyColor.Red]);
  ...
  const bobGreenCard = createTestCard(CardType.Property, 4, [PropertyColor.Green]);
  ...
  console.log("\nInitial state:");
  console.log(`Alice has: ${state.players[0].properties.map(s => `${s.color} (${s.cards.length})`).join(", ")}`);
  console.log(`Bob has: ${state.players[1].properties.map(s => `${s.color} (${s.cards.length})`).join(", ")}`);
  ...
  try {
    ...
    // Verify the swap
    const aliceHasGreen = state.players[0].properties.some(s => s.color === PropertyColor.Green);
    const bobHasRed = state.players[1].properties.some(s => s.color === PropertyColor.Red);
...
function testForceDeal4Player() {
  ...
  const state: GameState = {
    id: "test-game",
    players: [
      { id: "p1", name: "Alice", hand: [], bank: [], properties: [], connected: true },
      { id: "p2", name: "Bob", hand: [], bank: [], properties: [], connected: true },
      { id: "p3", name: "Charlie", hand: [], bank: [], properties: [], connected: true },
      { id: "p4", name: "Diana", hand: [], bank: [], properties: [], connected: true },
    ],
    deck: createDeck(),
    discardPile: [],
    currentPlayerIndex: 0,
  ...
  const aliceYellowCard = createTestCard(CardType.Property, 3, [PropertyColor.Yellow]);
  state.players[0].properties.push({
  ...
  const charlieOrangeCard = createTestCard(CardType.Property, 2, [PropertyColor.Orange]);
  ...
  console.log("\nInitial state:");
  console.log(`Alice has: ${state.players[0].properties.map(s => `${s.color} (${s.cards.length})`).join(", ")}`);
  console.log(`Charlie has: ${state.players[2].properties.map(s => `${s.color} (${s.cards.length})`).join(", ")}`);
  ...
  try {
    ...
    // Verify the swap
    const aliceHasOrange = state.players[0].properties.some(s => s.color === PropertyColor.Orange);
    const charlieHasYellow = state.players[2].properties.some(s => s.color === PropertyColor.Yellow);
...
function testForceDealWithJustSayNo() {
  ...
  try {
    ...
    console.log("\nFinal state:");
    console.log(`Alice still has: ${state.players[0].properties.map(s => `${s.color}`).join(", ")}`);
    console.log(`Bob still has: ${state.players[1].properties.map(s => `${s.color}`).join(", ")}`);
    
    // Verify no swap occurred
    const aliceStillHasPink = state.players[0].properties.some(s => s.color === PropertyColor.Pink);
    const bobStillHasRed = state.players[1].properties.some(s => s.color === PropertyColor.Red);
